...
Feature: Accessibility - Every shipped page is accessible
  ...
  # whose title came out as an h2 so there was no h1 to jump to. Both are named
  # here so a regression on the home page reads as the rule that broke.
  #
  ...
  @check @a11y @local @development @staging @production
  Scenario: The home page holds the two rules a sibling template regressed on
    Given I am an anonymous user
     When I go to "/"
      And I wait until the page is loaded
