 ...
 */
// A generous timeout on purpose: this step visits the page once per breakpoint
// and the first visit generates a drimage WebP derivative per rendered width,
// which on a cold CI site takes far longer than the default step timeout. The
// step asserts nothing, so a long ceiling cannot mask a failure.
When(/^(?:I |we )*warm up "([^"]*)" at all testing breakpoints$/, { timeout: 240000 }, async function (path) {
  const base = this.launchUrl.replace(/\/$/, '');
