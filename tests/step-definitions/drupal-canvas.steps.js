 ...
 */
When(/^(?:I |we )*open the "([^"]*)" Canvas page in the editor$/, { timeout: 300000 }, async function (pageTitle) {
  const id = await resolveCanvasPageId(this.page, pageTitle);
  ...
  // cucumber scenario retry. 90s per attempt because shared CI runners are far
  // slower to mount this than a dev machine (confirmed clean in ~8s locally).
  let ready = false;
  for (let attempt = 1; attempt <= 3 && !ready; attempt++) {
    try {
      await this.page.goto(editorUrl, { waitUntil: 'domcontentloaded' });
      await libraryButton.waitFor({ state: 'visible', timeout: 90000 });
      ready = true;
    } catch (e) {
